...
const mutations = {
	setRounds(s: ProgramsState, rounds: ProgramRound[]) { s.rounds = rounds },
	addRound(s: ProgramsState, round: ProgramRound) { s.rounds.push(round) },
	clear(s: ProgramsState) { s.rounds = [] }
}

function pickRandomExcluding(exclude: Set<string>, count: number, available: { name: string; condition: string; color: string }[]) {
...
const actions = {
	async generateAll({ commit, state, rootGetters, dispatch }: any) {
		...
		state.distances.forEach((distance: number, idx: number) => {
			const selected = pickRandomExcluding(lastRoundSet, state.totalHorsesPerRound, available)
	...
	regenerateRound({ commit, state }: any, roundNumber: number) {
		...
		if (state.rounds[index - 1]) {
			exclude = new Set(state.rounds[index - 1].horses.map((h: any) => h.name + '|' + h.color))
		}
		...
		const replacement = pickRandomExcluding(exclude, state.totalHorsesPerRound, currentPool.length ? currentPool : [])
		...
		for (let i = index + 1; i < state.distances.length; i++) {
			...
			const nextSelected = pickRandomExcluding(lastSet, state.totalHorsesPerRound, pool)
			newRounds[i] = {
				round: i + 1,
...
const getters = {
	...
	byRound: (s: ProgramsState) => (round: number) => s.rounds.find(r => r.round === round) || null,
