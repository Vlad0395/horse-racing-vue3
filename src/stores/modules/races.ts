...
function computeResultsByCondition(horses: any[] = [], distance?: number): RaceResult[] {
	if (!horses.length) return []
	// Parse numeric condition (fallback 50) and keep original ref
	const enriched = horses.map(h => {
		const cRaw = (h && (h.condition ?? h.conditional ?? h.power)) as string | number | undefined
	...
	const spreadFactor = 0.10 // up to 10% improvement window for best vs worst
	...
	return computed.map((c, idx) => ({ horse: c.horse, position: idx + 1, timeMs: c.timeMs }))
}
		...
		const race = state.races.find(r => r.id === state.currentRaceId)
			if (race && !race.finishedAt) {
				...
				if (!race.results || !race.results.length) {
					race.results = computeResultsByCondition(race.horses as any[], race.distance)
				}
			}
		state.active = false
		state.currentRaceId = null
	},
	setSequenceRunning(state: RacesState, val: boolean) { state.sequenceRunning = val },
	setCurrentRoundIndex(state: RacesState, idx: number | null) { state.currentRoundIndex = idx },
	setSequenceTimer(state: RacesState, id: number | null) { state.sequenceTimerId = id },
	setGap(state: RacesState, ms: number) { state.gapMs = ms },
	setRaceDuration(state: RacesState, ms: number) { state.raceDurationMs = ms }
}
...
const actions = {
  ...
  cancelProgramSequence({ state, commit }: { state: RacesState; commit: Function }) {
	...
	_advanceSequence({ state, commit, dispatch, rootGetters }: any) {
		...
		commit('startRace', { programRound: round.round, distance: round.distance, horses: round.horses })
		const finalize = () => {
			commit('finishRace')
			const nextIdx = idx + 1
		...
		if (state.raceDurationMs > 0) {
			setTimeout(finalize, state.raceDurationMs)
		} else {
			finalize()
		}
	}
